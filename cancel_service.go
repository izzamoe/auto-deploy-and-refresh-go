...
func NewCancelService(q *DeployQueue) *CancelService {
	return &CancelService{q: q}
}

func (s *CancelService) SetEventHub(events *AdminEventHub) {
	if s != nil {
		s.events = events
	}
}

func NewDeployControl(q *DeployQueue) *DeployControl {
	return &DeployControl{CancelService: NewCancelService(q)}
}
...
func (s *CancelService) publishCancelResult(result CancelJobResult) {
	...
	s.events.Broadcast(AdminEvent{
		Type:  adminEventTypeCancelRequested,
		AppID: result.AppID,
		JobID: result.JobID,
...
func getJobForCancel(tx *sql.Tx, jobID string) (JobRecord, error) {
	var job JobRecord
