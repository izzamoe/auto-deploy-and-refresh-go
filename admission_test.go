...
func newTestAdmission(t *testing.T, maxPending int) (*AdmissionService, *AppStore, *DeployQueue) {
	t.Helper()
	db := newTestDB(t)
	...
	queue, err := NewDeployQueue(db, maxPending)
	if err != nil {
		t.Fatalf("NewDeployQueue: %v", err)
	}

	svc := NewAdmissionService(store, queue)
