...
func enqueueJob(t *testing.T, q *DeployQueue, appID, tag string) string {
	t.Helper()
...
func dequeueJob(t *testing.T, q *DeployQueue, appID, tag string) string {
	t.Helper()
	if err := q.Enqueue(appID, tag); err != nil {
		t.Fatalf("Enqueue(%s, %s): %v", appID, tag, err)
	}
...
func insertJobWithStatus(t *testing.T, q *DeployQueue, appID, tag, status string) string {
	t.Helper()
...
func assertJobStatus(t *testing.T, q *DeployQueue, jobID, want string) {
	t.Helper()
	var status string
