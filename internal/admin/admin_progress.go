...
func NewProgressAdminHandler(tracker *progress.ProgressTracker, store *store.AppStore, queue *store.DeployQueue, templates map[string]*template.Template) *ProgressAdminHandler {
...
func (h *ProgressAdminHandler) matchingProgressSnapshots(r *http.Request) []progress.ProgressSnapshot {
	...
	matched := make([]progress.ProgressSnapshot, 0, len(snapshots))
...
func progressSnapshotFrame(snap progress.ProgressSnapshot) (string, error) {
	return progress.EncodeProgressFrame(progress.ProgressFrame{
		AppID:           snap.AppID,
		JobID:           snap.JobID,
		Tag:             snap.Tag,
		Stage:           snap.Phase,
...
func progressStatusForStage(stage string) string {
	switch stage {
	case progress.ProgressStageQueued:
		return progress.ProgressStatusPending
	case progress.ProgressStageSucceeded:
		return progress.ProgressStatusSucceeded
	case progress.ProgressStageFailed:
		return progress.ProgressStatusFailed
	default:
		return progress.ProgressStatusInProgress
	}
}
