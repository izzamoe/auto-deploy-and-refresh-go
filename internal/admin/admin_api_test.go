...
func TestAdminAPIManualDeployQueuesJSON(t *testing.T) {
	...
	if len(jobs) != 1 || jobs[0].Trigger != "manual_deploy" || jobs[0].Status != cancel.JobStatusPending {
...
func TestAdminAPICancelEndpoints(t *testing.T) {
	...
	if jobCancel.Result.Status != cancel.JobStatusCanceled || jobCancel.Result.Outcome != cancel.CancelOutcomePendingCanceled {
	...
	if status != cancel.JobStatusCancelRequested {
...
func TestAdminAPIAppCancelReturnsCounts(t *testing.T) {
	...
	enqueueJob(t, queue, app.ID, "v-active")
	activeID := dequeueJob(t, queue, app.ID, "v-active")
	enqueueJob(t, queue, app.ID, "v-terminal")
	...
	assertJobStatus(t, queue, pendingID, cancel.JobStatusCanceled)
	assertJobStatus(t, queue, activeID, cancel.JobStatusCancelRequested)
	assertJobStatus(t, queue, terminalID, cancel.JobStatusSucceeded)
	assertJobStatus(t, queue, otherAppID, cancel.JobStatusPending)
}
